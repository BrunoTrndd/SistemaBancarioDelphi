...
uses Generics.Collections,SysUtils, uPRODUTO, uCONTA, uDINHEIRO, uIInterfaceBanco, uPROGRAMA,uMovimentacao;
...
type
  ...
  public
    ...
    procedure MovimentarDinheiro();
    procedure CadastrarContaProduto();
    procedure Menu();
...
procedure TBanco.CadastrarContaProduto();
var
vProduto : TProduto;
vConta   : TConta;
begin
  ...
  Writeln('Qual produto deseja utilizar?');
  Readln(vTexto);
  vProduto := GetProduto(vTexto);

  Writeln('Qual conta deseja vincular o produto?');
  Readln(vTexto);
  vConta := GetConta(StrToInt(vTexto));
  ...
  Writeln('O produto: ' +vProduto.Nome+ ' Foi cadastrado na conta: ' + IntToStr(vConta.Numero));
end;
...
procedure TBanco.Menu;
begin
  Writeln('----------------------------------------' +sLineBreak+
          '20 - Deposito                           ' +sLineBreak+
          '21 - Saque                              ' +sLineBreak+
          '----------------------------------------' +sLineBreak
          );
end;

procedure TBanco.MovimentarDinheiro;
var
vEscolha : integer;
...
begin
try
vMovimentacao := TMovimentacao.Create(Dinheiros);
...
case vEscolha of
         20:
         begin
           writeln('Qual produto deseja movimentar?');
           readln(vTexto);
           vProduto := GetProduto(vTexto);

           writeln('Qual a conta que deseja movimentar?');
           readln(vTexto);
           vConta := GetConta(StrToInt(vTexto));

           if not vConta.ExisteProduto(vProduto.Nome) then
           begin
             raise Exception.Create('Esse produto nao existe nessa conta!');
           end else if vConta.ProdutoValido(vProduto) then
           begin
             Writeln('Quanto deseja depositar?');
             Readln(vTexto);
             vConta.GetContaProduto(vProduto.Nome).IncrementarSaldo(StrToCurr(vTexto));
...
//                                                                 writeln('Quanto deseja depositar?');
//           vMovimentacao.RealizarDeposito(
         end;
         ...
         begin
           Writeln('Digite um valor valido, para retornar ao menu, digite 0');
         end;
